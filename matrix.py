...
if __name__ == '__main__':
    ...
    op = listkron( [ss.ops['I'], ss.ops['X'] ] ) 
    js = [0,1]

    init_state = ss.make_state(L, IC)
    final_state = op_on_state(op, js, init_state)
    final_state2 = big_mat([ ss.ops['I'], ss.ops['X'] ], js, init_state)
    ...
    Z_grid = [[ms.n_j(state, j, proj='1') for j in range(L)] \
            for state in (init_state, final_state, final_state2)]
    print(Z_grid) 
    #comp_plot()
