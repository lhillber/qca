...
def time_evolve(Rs, IC, L, tmax):
    ...
    print('xsi gen', np.mean([t2- t1 for t1, t2 in zip(ta, tb) ]))
    print('j loop', np.mean([t2- t1 for t1, t2 in zip(tb, tc) ]))
    print('trace out', np.mean([t2- t1 for t1, t2 in zip(tc, td) ]))
